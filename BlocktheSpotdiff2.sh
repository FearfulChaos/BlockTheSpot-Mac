...
show_progress() {
    ...
    # Print the bar, percentage, and message on the same line
    printf "\r[%-${bar_width}s] %3d%% - %s" "$(printf '#%.0s' $(seq 1 $filled))$(printf '.%.0s' $(seq 1 $empty))" "$percent" "$message"
}
...
if [[ ! -f "${XPUI_SPA}" ]]; then
  ...
  if [[ "${FORCE_FLAG}" == "false" ]]; then
    if [[ -f "${XPUI_BAK}" ]] || [[ -f "${APP_BINARY_BAK}" ]]; then
      ...
      CURRENT_STEP=$((CURRENT_STEP + 1)); show_progress "Creating backup" $TOTAL_STEPS $CURRENT_STEP
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  CURRENT_STEP=$((CURRENT_STEP + 1)); show_progress "Extracting xpui" $TOTAL_STEPS $CURRENT_STEP
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  CURRENT_STEP=$((CURRENT_STEP + 1)); show_progress "Patching ads (JS)" $TOTAL_STEPS $CURRENT_STEP
  $PERL "${AD_ADS}" "${XPUI_JS}"
  ...
  CURRENT_STEP=$((CURRENT_STEP + 1)); show_progress "Patching binary" $TOTAL_STEPS $CURRENT_STEP
  ...
  CURRENT_STEP=$((CURRENT_STEP + 1)); show_progress "Removing premium features" $TOTAL_STEPS $CURRENT_STEP
...
if [[ "${XPUI_SKIP}" == "false" ]]; then
  CURRENT_STEP=$((CURRENT_STEP + 1)); show_progress "Re-zipping xpui.spa" $TOTAL_STEPS $CURRENT_STEP
  (cd "${XPUI_DIR}"; zip -qq -r ../xpui.spa .)
  rm -rf "${XPUI_DIR}"; fi

# Sign APP_BINARY
CURRENT_STEP=$((CURRENT_STEP + 1)); show_progress "Signing app" $TOTAL_STEPS $CURRENT_STEP
